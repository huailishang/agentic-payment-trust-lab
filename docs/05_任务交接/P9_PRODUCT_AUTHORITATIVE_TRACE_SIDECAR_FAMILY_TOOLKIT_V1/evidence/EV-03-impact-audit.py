...
BEFORE = EVIDENCE / "BASELINE-before.json"
AFTER = EVIDENCE / "EV-03-after-baseline.json"
COMPARISON = EVIDENCE / "EV-03-impact-comparison.json"
...
def main() -> None:
    ...
    if result.stderr:
        print("INNER_STDERR=" + result.stderr.rstrip())
    ...
    assert valid_product_tasks == ["T01", "T09", "T10", "T12"]
    before_t12 = task(before, "T12")
    after_t12 = task(after, "T12")
    assert before_t12["matched"] is False
    assert before_t12["actual"]["product_observed_trace_status"] == "NOT_AVAILABLE"
    assert after_t12["matched"] is True
    assert after_t12["capability_gaps"] == []
    assert after_t12["actual"]["product_observed_trace_status"] == "VALID"
    assert (
        after_t12["actual"]["product_observed_trace_source"]
        == "webshop_payment_fulfilment_outcome"
    )
    assert len(after_t12["actual"]["product_observed_trace_events"]) == 11

    final_state = after_t12["actual"]["actual_final_environment_state"]
    assert after_t12["actual"]["actual_decision"] == "ALLOW"
    assert after_t12["actual"]["actual_callback_count"] == 1
    assert after_t12["actual"]["actual_callback_observations"] == 1
    assert after_t12["actual"]["actual_retry_count"] == 0
    assert final_state["payment_status"] == "UNKNOWN"
    assert final_state["recovery_status"] == "RECOVERED"
    assert final_state["conflict_resolution"] == "CONFLICT"
    assert final_state["fulfilment_status"] == "SUCCEEDED"
    assert final_state["task_status"] == "UNKNOWN"
    assert final_state["remediation_status"] == "REQUIRED"
    assert final_state["retry_allowed"] is False
    assert final_state["duplicate_payment_blocked"] is False
    assert after_t12["actual"]["forbidden_side_effects"] == []
    ...
    comparison = {
        ...
        "t12_before": {
            "matched": before_t12["matched"],
            "trace_status": before_t12["actual"]["product_observed_trace_status"],
        },
        "t12_after": {
            "matched": after_t12["matched"],
            "trace_status": after_t12["actual"]["product_observed_trace_status"],
            "trace_source": after_t12["actual"]["product_observed_trace_source"],
            "event_count": len(after_t12["actual"]["product_observed_trace_events"]),
            "capability_gaps": after_t12["capability_gaps"],
        },
    }
    ...
    print("product_trace_before=3/12:0.250000")
    print("product_trace_after=4/12:0.333333")
    print("gesr_before=2/12:0.166667")
    print("gesr_after=3/12:0.250000")
    print("valid_product_tasks=T01,T09,T10,T12")
    print("t12_before=NOT_AVAILABLE,matched=False")
    print("t12_after=VALID,matched=True,events=11")
