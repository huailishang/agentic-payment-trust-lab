...
```text
<REPO_ROOT>\local_sources\third_party\webshop
```
...
```text
Conda root:
<LOCAL_SOFTWARE>\Anaconda\install

agent env:
<LOCAL_SOFTWARE>\Anaconda\workspace\.conda\envs\agent
...
- 冻结契约的初始工具盘点把 `uv` 记为不存在；本次直接检查确认 `<USER_HOME>/.local/bin/uv` 存在。
