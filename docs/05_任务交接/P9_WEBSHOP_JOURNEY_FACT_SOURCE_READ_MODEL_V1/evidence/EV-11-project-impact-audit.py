...
def main() -> None:
    ...
    assert metrics["product_observed_authoritative_trace_completeness_rate"]["count"] == 9
    assert metrics["product_observed_authoritative_trace_completeness_rate"]["denominator"] == 12
    assert metrics["governed_end_to_end_task_success_rate"]["count"] == 8
    assert metrics["governed_end_to_end_task_success_rate"]["denominator"] == 12
    assert metrics["callback_count_match_rate"]["count"] == 12
    assert metrics["callback_count_match_rate"]["denominator"] == 12
    assert metrics["duplicate_or_forbidden_side_effect_rate"]["count"] == 0
    assert metrics["duplicate_or_forbidden_side_effect_rate"]["denominator"] == 12
    valid = sorted(
        task_id
