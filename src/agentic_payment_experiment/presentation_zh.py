...
REASON_PRESENTATION_ZH = {
    ...
    "confirmation_binding_invalid": ("原确认已经失效", "当前授权版本或交易内容已经不能与用户保存的确认可靠绑定。", "展示变化并重新取得用户确认。"),
    "confirmation_binding_missing_evidence": ("确认绑定证据缺失", "缺少确认记录、当前订单或授权版本引用，无法证明仍是用户确认的交易。", "停止执行并补齐可信确认记录。"),
    ...
    "payment_execution_binding_missing_evidence": ("支付执行绑定证据缺失", "缺少授权、订单、支付请求或计划执行记录的必要连续引用。", "停止执行并补齐可信引用。"),
    "payment_execution_binding_invalid": ("支付执行连续绑定无效", "计划执行记录不能可靠回溯到刚刚核验的支付请求和订单。", "停止执行并重新生成正确绑定的支付记录。"),
    "payment_request_binding_mismatch": ("支付执行指向了其他请求", "计划执行记录没有指向刚刚通过付款前检查的请求。", "停止执行并核对支付请求引用。"),
    "payment_order_binding_mismatch": ("支付链指向了其他订单", "支付请求或计划执行记录没有指向当前订单。", "停止执行并核对订单引用。"),
    "payment_authority_binding_mismatch": ("支付链授权引用不一致", "订单、支付请求或计划执行记录没有保持当前授权及其版本。", "停止执行并重新建立授权链。"),
    "payment_agent_binding_mismatch": ("支付链智能体引用不一致", "支付请求或计划执行记录没有保持委托指定的智能体引用。", "停止执行并核对执行主体。"),
    "payment_payee_binding_mismatch": ("支付链收款方不一致", "支付请求或计划执行记录中的收款方与订单不一致。", "停止执行并核实收款主体。"),
    "payment_amount_binding_mismatch": ("支付链金额不一致", "计划执行金额没有保持订单和已核验支付请求的金额。", "停止执行并重新生成支付请求。"),
    "payment_currency_binding_mismatch": ("支付链币种不一致", "计划执行币种没有保持订单和已核验支付请求的币种。", "停止执行并重新生成支付请求。"),
    "payment_merchant_binding_mismatch": ("支付请求商户不一致", "支付请求中的商户与当前订单不一致。", "停止执行并核对商户。"),
    "payment_execution_identity_invalid": ("支付执行标识或时序无效", "支付编号与请求/订单发生冲突，或执行时间早于请求。", "停止执行并修复执行标识或时序。"),
}
...
_TRUSTED_EXECUTION_STAGE_BLUEPRINT = {
    ...
    "L5": {
        "relevant": True,
        "status_code": "PROTOTYPE",
        "status_zh": "已接入 S10-S12",
        "capabilities": ["Continuous Binding", "Execution Identity", "Idempotency"],
        "purpose_zh": "验证订单、支付请求与计划执行记录是否属于同一授权交易链，以及唯一执行边界是否存在。",
        "current_state_zh": "P2 已在 S10-S12 消费 Order → Payment Request → Payment Execution 连续绑定；S12 继续消费幂等边界和同请求尝试清单。",
        "returns_zh": "返回连续引用、关键支付字段、执行标识及幂等边界等确定性事实。",
        "does_not_decide_zh": "连续绑定有效只代表执行对象一致；最终是否执行及是否可重试仍由 Payment Domain 决定。",
    },
    "L6": {
        "relevant": True,
        "status_code": "PROTOTYPE",
